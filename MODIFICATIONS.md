...
本次修改目的：保留上游现有功能，作为独立前端扩展增加 WaveSpeed 和 Civitai 文生图支持；无需服务端插件。版本 3.1.0-cloud.8。
...
分开两家表单与保存校验，增加固定保存栏、实际启用平台标记、修改状态、数据库读回核对及配置版本。新请求先保存当前平台修改，失败时不沿用旧值；设置导出先保存编辑内容。默认 JSON 尺寸优先，保留显式正文尺寸映射选项。显示保存后的 LoRA 清单，以及任务入队快照和实际请求参数；历史任务折叠，增加按当前配置重新生成入口。定时刷新保留任务详情展开状态与恢复 ID 输入。原世界书、LLM、任务队列和图片插入模块不变。
