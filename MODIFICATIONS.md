...
本次修改目的：保留上游现有功能，作为独立前端扩展增加 WaveSpeed 和 Civitai 文生图支持；无需服务端插件。版本 3.1.0-cloud.9。
...
新增 wavespeed/migration.js，仅对明确标识的 v2 私密迁移文件恢复凭据与云端参数；v1 普通导入继续保留目标设备云端 Key。校验格式后写入插件独立 IndexedDB，不迁移任务或浏览器身份。保持目标设备已有世界书文件和词库不变。导入完成提示刷新，失败显示实际原因。个人文件只放本地 dist，不提交仓库。

cloud.9 同时取消云端设置大面板的 sticky 定位，压缩手机标题和导航，在两家 JSON 下方提供保存按钮与状态，保存或解析失败就地显示原因；复用原保存与读回核对流程。
