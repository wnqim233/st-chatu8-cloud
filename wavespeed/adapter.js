...
export function createWaveSpeedAdapter(deps) {
  ...
  async function generate(request) {
    ...
    try {
      ...
      deps.taskQueue.updateStatus(taskId, 'running');
      ...
      remember(first);
      const job = await wait(api, first, { onUpdate: remember });
      ...
      if (taskId) deps.taskQueue.completeTask(taskId, false);
  ...
  function updateMode() {
    ...
    if (enabled) deps.initializeImages();
  }
  ...
  async function refreshJobs() {
    jobs = await api('/jobs');
    for (const job of [...jobs]) if (['submitting', 'submitted', 'processing', 'download_failed'].includes(job.status)) remember(await api(`/jobs/${job.id}/refresh`, {}));
    renderJobs(); notify('任务状态已更新。未完成的任务可稍后继续查询，不会重新提交生图。');
  }
  function renderJobs() {
    if (!$('jobs')) return;
    ...
    for (const job of jobs) {
      ...
      card.append(node('strong', LABELS[job.status] || job.status), node('p', `${new Date(job.createdAt).toLocaleString()} · ${job.model}`, 'ws-hint'));
      ...
      if (job.status === 'unknown' || job.status === 'submitting' && Date.now() - job.createdAt > 90000) {
        ...
        if (Date.now() - job.createdAt > 90000) {
