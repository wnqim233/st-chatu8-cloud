...
**版本 3.1.0-cloud.11 不需要安装服务端插件，不需要修改 config.yaml。** 原智绘姬可以保留安装；使用这版前，在酒馆扩展管理中停用原版并刷新页面。两版使用独立配置与缓存，目前不支持同时启用。

cloud.11 明确关闭 Creator Tip 和 Civitai Tip：报价与生成均传 0；平台若仍报非零小费就阻止提交。基础生成费和资源授权费仍以报价为准。

cloud.10 新增 Buzz 币种选择：默认仅黄 Buzz 并提前允许成熟内容，也可选择仅蓝、仅绿或蓝＋绿（仅 SFW）。所有新任务明确禁止自动升级扣费，报价与提交使用同一支付设置；任务卡保留费用与交易明细。详见 [Buzz 计费核实与使用说明](docs/CIVITAI_BUZZ.md)。

cloud.9 修复手机保存栏遮挡：面板随页面滚动，JSON 下方可直接保存并查看成功或失败状态，手机标题和导航占用更小。

cloud.9 支持私密完整迁移文件：通过原设置导入按钮，一次恢复云端 Key、模型和 LoRA 参数、插件设置和世界书勾选项。导入后刷新页面。此文件含密钥，不要公开；普通导出仍不含云端 Key。

cloud.8 整理了云端设置页：分平台保存、未保存提示、保存后核对、LoRA 清单和每次任务的实际请求参数；修改后直接生图会先保存，出错时停止提交。默认按 JSON 尺寸，可切换跟随单图尺寸。
...
Key 与任务保存在当前浏览器中，更换浏览器时可使用私密设置迁移文件；没有备份则需重新设置。世界书继续使用酒馆已有世界书，在本插件“发送数据”页选择；原版设置不自动搬入，需要时使用原有导入/导出功能。
